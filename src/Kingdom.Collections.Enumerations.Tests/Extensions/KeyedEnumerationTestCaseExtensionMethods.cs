...
namespace Kingdom.Collections
{
    ...
    /// <see cref="KeyedEnumerationTestFixtureBase{TKey,T}"/> tests. Serves for both
    /// Ordinal as well as <see cref="FlagsEnumerationBase{T}"/> based test cases.
    /// </summary>
    public static class KeyedEnumerationTestCaseExtensionMethods
    {
        ...
        /// connect the caller with the <see cref="Enumeration{TKey, T}"/>
        ...
        {
            var value = Enumeration<TKey, T>.FromKey(key).AssertNotNull();
            verify?.Invoke(value.AssertEqual(key, x => x.Key));
            return value;
        }

        /// <summary>
        /// <typeparamref name="T"/> <see cref="FlagsEnumerationBase{T}"/> values Shall All have
        /// Consistent <see cref="Enumeration{TKey}.Key"/> lengths. Rules out false positives in
        /// the form of empty <see cref="Enumeration.GetValues{T}"/>.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        ...
        /// <see cref="Enumeration{TKey,T}"/> type only. Additionally, we are making
