...
namespace Kingdom.Collections
{
    ...
    /// <summary>
    /// <see cref="BaseEnumeration{T}"/> coverage reporter.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <inheritdoc />
    public interface IBaseEnumerationCoverageReporter<in T> : IDisposable
    ...
    /// <summary>
    /// Implements the <see cref="BaseEnumeration{T}"/> coverage reporter.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <inheritdoc />
    public class BaseEnumerationCoverageReporter<T> : IBaseEnumerationCoverageReporter<T>
        where T : BaseEnumeration<T>
    {
        /// <summary>
        /// Gets or sets whether the Reporter is considered Enabled for purposes
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (disposing && !IsDisposed)
            {
                if (Enabled)
                {
                    BaseEnumeration<T>.Values.VerifyBaseValuesCoverage(TestedNames);
                }
            }
        }
    ...
    /// </summary>
    public static class BaseReporterExtensionMethods
    {
        internal static void VerifyBaseValuesCoverage<T>(this IEnumerable<T> values, IDictionary<string, int> coverage)
            where T : BaseEnumeration<T>
        {
